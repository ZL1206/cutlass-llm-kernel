...
constexpr int MAX_ALL_REDUCE_BLOCKS = 36;
...
struct AllReduceParams
{
    ...
    T* peer_comm_buffer_ptrs[MAX_RANKS_PER_NODE];
...
__inline__ __device__ void multi_gpu_barrier(uint32_t** signals, const uint32_t flag, const int local_rank,
    const int world_size, const int tidx, const int bidx)
{
    ...
    if (tidx < world_size)
    {
        ...
        if (bidx == 0)
        {
            st_flag_volatile(flag, signals[tidx] + local_rank);
...
__forceinline__ __device__ void block_barrier(uint32_t** signals, const uint32_t flag, const int local_rank,
    const int world_size, const int tid, const int bid, const int grid_size)
{
    __syncthreads();
    ...
    if (tid < world_size)
    {
        ...
        uint32_t flag_block_offset = bid * world_size;

        
        st_flag_release(flag, signals[tid] + flag_block_offset + local_rank);

        // Blocks check that corresponding blocks on other GPUs have also set the flag
        uint32_t* peer_barrier_d = signals[local_rank] + flag_block_offset + tid;
...
static __global__ void twoshot_allreduce_kernel(AllReduceParams<T> params)
{
    ...
    for (int idx = tid; idx < vec_nums; idx += stride) {
        ...
        local_shared_buffer[offset] = *reinterpret_cast<float4*>(result.data());
    }
...
template <typename T, int WORLD_SIZE>
static __global__ void twoshot_allreduce_fp8_kernel(AllReduceParams<T> params)
{
    const int grid_size = gridDim.x;
    const int block_size = blockDim.x;
    const int stride = grid_size * block_size;
    ...
    int base = ((params.elts_total / WORLD_SIZE) / params.group_size) * params.group_size;
    int remainder = (params.elts_total - base * WORLD_SIZE) / params.group_size;
    int elts = params.local_rank < remainder ? base + params.group_size : base;
    int largest_elts = remainder > 0 ? base + params.group_size : base;
    int part = elts / vec_size;
    int largest_part = largest_elts / vec_size;
    int start = (base * params.local_rank + min(remainder, params.local_rank) * params.group_size) / vec_size;
    int end = start + part;
    /*
    if (threadIdx.x == 0) {
    printf("rank %d, base %d, remainder %d, elts %d, largest_elts %d, part %d, largest_part %d, start %d, end %d\n", params.local_rank, base, remainder, elts, largest_elts, part, largest_part, start, end);
    }
    */
    ...
    for (int idx = tid; idx < vec_nums; idx += stride) {
        ...
        for (int mask = 8 / 2; mask >= 1; mask /= 2) {
            max_abs_val = fmaxf(max_abs_val, __shfl_xor_sync(uint32_t(-1), max_abs_val, mask, 8));
        }
    ...
    //printf("rank %d, arrive second barrier\n", params.local_rank);
    ...
    for (int idx = tid; idx < largest_part; idx += stride) {
        ...
        for (int i = 0; i < WORLD_SIZE; ++i)
        {
            ...
            const int start = (base * rank + min(remainder, rank) * params.group_size) / vec_size;
            const int offset = start + idx;
            const int scale_offset = (offset * vec_size) / params.group_size;
            if (rank < remainder || idx < part) {
                Vec val;
...
template <typename T, int WORLD_SIZE>
__global__ void oneshot_allreduce_kernel(AllReduceParams<T> params)
{
    const int grid_size = gridDim.x;
    const int block_size = blockDim.x;
    const int stride = grid_size * block_size;
...
template <typename T, int WORLD_SIZE>
__global__ void oneshot_allreduce_fp8_kernel(AllReduceParams<T> params)
{
    const int grid_size = gridDim.x;
    const int block_size = blockDim.x;
    const int stride = grid_size * block_size;
...
template <typename T>
void run(int batch_size, int hidden_size, int world_size, int rank, T** buffer_ipc_ptrs, float** scale_ipc_ptrs, uint32_t** barrier_in_ipc_ptrs, uint32_t** barrier_out_ipc_ptrs, ncclComm_t& comm, bool fp8) {
    int warmup = 10;
    int iter = 1000;
    int head_size = 128;
    ...
    fp8_blocks_per_grid = std::min(MAX_ALL_REDUCE_BLOCKS, fp8_blocks_per_grid);
    int total_threads = message_size / 8;
    ...
    */
    ...
    for (int i = 0; i < warmup; i++) {
        ...
        allreduce_fp8_quantize<T><<<grid, block, 0, stream>>>(d_in, out, params.fp8_scale_ptrs[params.local_rank]);
        switch (world_size) {
            case 2: twoshot_allreduce_fp8_kernel<T, 2><<<fp8_blocks_per_grid, fp8_threads_per_block, 0, stream>>>(params); break;
            case 4: twoshot_allreduce_fp8_kernel<T, 4><<<fp8_blocks_per_grid, fp8_threads_per_block, 0, stream>>>(params); break;
            case 8: twoshot_allreduce_fp8_kernel<T, 8><<<fp8_blocks_per_grid, threads_per_block, 0, stream>>>(params); break;
            default: break;
        }
    }
...
int main(int argc, char** argv) {
    ...
    CUDACHECK(cudaMalloc(&inter_buffer, buffer_size_in_bytes));
    ...
    // scales buffer
    float* scales;
    int scales_buffer_size = 1024 * 8192 / (128 * 2) * sizeof(float);
    ...
    */
    ...
    int hidden_size = 10240;
    std::vector<int> batch_size{1, 2, 4, 8, 16, 32, 64, 128, 256, 512};
    for (int bs : batch_size) {
        run<T>(bs, hidden_size, world_size, rank, buffer_ipc_ptrs, scale_ipc_ptrs, barrier_in_ipc_ptrs, barrier_out_ipc_ptrs, comm, true);
    }
    MPI_Barrier(MPI_COMM_WORLD);
    cudaFree(buffer);
    cudaFree(scales);
