...
struct Mask {
    ...
    // Causal_mask: whether this particular iteration needs causal masking
    template <bool Causal_mask=false, bool Is_even_MN=true, typename Engine, typename Layout>
    __forceinline__ __device__ void apply_mask_split_kv(Tensor<Engine, Layout> &tensor_,
                                                        const int col_idx_offset_,
                                                        const int row_idx_offset_,
                                                        const int tid,
                                                        const int mma_m) {
        ...
        if constexpr (Need_masking) {
            ...
            const int col_idx_offset = col_idx_offset_ + warp_idx * size<1,1>(tensor) * 8 + (lane % 4) * 2;
            if constexpr (Col_idx_only) {
                #pragma unroll
                for (int nj = 0; nj < size<1, 1>(tensor); ++nj) {
                    const int col_idx_base = col_idx_offset + nj * 8;
                ...
                #pragma unroll
                for (int mi = 0; mi < size<0 ,1>(tensor); mi++) {
                    const int row_idx_base = row_idx_offset_ + mi * mma_m + lane / 4;
                    #pragma unroll
                    for (int i = 0; i < size<0, 0>(tensor); i++) {
                        const int row_idx = row_idx_base + i * 8;
                        const int col_idx_limit_right = std::min(max_seqlen_k, row_idx + 1 + max_seqlen_k - max_seqlen_q);
