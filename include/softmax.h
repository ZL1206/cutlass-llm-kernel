...
__device__ __forceinline__ void thread_reduce_(Tensor<Engine0, Layout0> const &tensor, Tensor<Engine1, Layout1> &summary, Operator &op) {
    ...
    for (int mi = 0; mi < size<0>(tensor); mi++) {
        ...
        summary(mi) = op(summary(mi), tensor(mi, 0));
        #pragma unroll
...
__device__ __forceinline__ void reduce_(Tensor<Engine0, Layout0> const& tensor, Tensor<Engine1, Layout1> &summary, Operator &op) {
    thread_reduce_(tensor, summary, op);
    if (thread0()) {
        printf("thread max is: \n");
...
struct Softmax {
    ...
    template<bool Check_inf=false, typename Tensor0, typename Tensor1>
    __forceinline__ __device__ void softmax_rescale_o(Tensor0 &acc_s, Tensor1 &acc_o, float softmax_scale_log2) {
        // Reshape acc_s from (MMA=4, MMA_M, MMA_N) to (nrow=(2, MMA_M), ncol=(2, MMA_N))
        Tensor scores = make_tensor(acc_s.data(), flash::convert_layout_acc_rowcol(acc_s.layout()));
        Tensor acc_o_rowcol = make_tensor(acc_o.data(), flash::convert_layout_acc_rowcol(acc_o.layout()));
        if (thread0()) {
            print("scores: "); print(scores); printf("\n");
            print("acc_o_rowcol: "); print(acc_o_rowcol); print("\n");
        }
        static_assert(decltype(size<0>(scores))::value == kNRows);
        static_assert(decltype(size<0>(acc_o_rowcol))::value == kNRows);
        Tensor scores_max_prev = make_fragment_like(row_max);
        cute::copy(row_max, scores_max_prev);
        flash::template reduce_max(scores, row_max);
        #pragma unroll
