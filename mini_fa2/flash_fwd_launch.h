...
__global__ void mini_flash_fwd_kernel(__grid_constant__ const kernel_params params) {
    ...
    if (Is_causal) {
        n_block_max = cute::min(n_block_max, cute::ceil_div((m_block + 1) * kBlockM + params.seqlen_k - params.seqlen_q, kBlockN));
    }
    ...
    if ((Is_causal || !Is_even_MN) && n_block_max <= n_block_min) {
        ...
        Tensor gO = local_tile(mO, Shape<Int<kBlockM>, Int<kHeadDim>>{},
        ...
        flash::copy<Is_even_MN, Is_even_K, /*Clear_OOB_MN=*/false, /*Clear_OOB_K=*/false>(
            gmem_tiled_copy_O, tOrO, tOgO, tOcO, tOpO, params.seqlen_q - m_block * kBlockM
        );
        return;
    }
    

    Tensor mQ = make_tensor(make_gmem_ptr(reinterpret_cast<T*>(params.q_ptr)),
                            make_shape(params.seqlen_q, params.d),
                            make_stride(params.q_head_stride, _1{}));
    Tensor gQ = local_tile(mQ, Shape<Int<kBlockM>, Int<kHeadDim>>{},
                        make_coord(m_block, 0));  // (kBlockM, kHeadDim)
    
    Tensor mK = make_tensor(make_gmem_ptr(reinterpret_cast<T*>(params.k_ptr)),
                            make_shape(params.seqlen_k, params.d),
                            make_stride(params.k_head_stride, _1{}));
    
    Tensor gK = local_tile(mK, Shape<Int<kBlockN>, Int<kHeadDim>>{},
                        make_coord(_, 0));  // (kBlockN, kHeadDim, n)
    
    Tensor mV = make_tensor(make_gmem_ptr(reinterpret_cast<T*>(params.v_ptr)),
                            make_shape(params.seqlen_k, params.d),
                            make_stride(params.v_head_stride, _1{}));
    ...
    if (!Is_causal && !Is_even_MN) {
        ...
        n_masking_steps = n_block_max - cute::max((m_block * kBlockM + params.seqlen_k - params.seqlen_q) / kBlockN, 0);
    }
    if (thread0()) {
    ...
    for (; n_block >= n_block_min; --n_block) {
        ...
        if (n_block == n_block_max - 1) {
            flash::copy<Is_even_MN, Is_even_K, /*Clear_OOB_MN=*/true>(gmem_tiled_copy_QKV, tVgV(_, _, _, n_block), tVsV, tKVcKV, tKVpKV, params.seqlen_k - n_block * kBlockN);
        } else {
...
void run_mini_flash_fwd(kernel_params &params, cudaStream_t stream) {
    ...
    BOOL_SWITCH(is_even_MN, IsEvenMNConst, [&] {
        BOOL_SWITCH(is_even_K, IsEvenKConst, [&] {
            ...
            auto kernel = &mini_flash_fwd_kernel<Kernel_traits, Is_causal, IsEvenMNConst && IsEvenKConst && Kernel_traits::kHeadDim <= 128, IsEvenKConst>;
...
void run_mini_mha_fwd_128(kernel_params &params, cudaStream_t stream) {
    ...
    if (is_sm8x) {
        if constexpr(!Is_causal) {
            run_mini_flash_fwd<Kernel_traits<T, 128, 32, Headdim, 4>, Is_causal>(params, stream);
        } else {
            run_mini_flash_fwd<Kernel_traits<T, 64, 64, Headdim, 4>, Is_causal>(params, stream);
        }
    } else {
        run_mini_flash_fwd<Kernel_traits<T, 128, 64, Headdim, 4>, Is_causal>(params, stream);
    }
