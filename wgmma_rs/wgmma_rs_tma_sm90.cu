...
void gemm_device(ProblemShape shape_MNK,
    ...
    if (thread0()) {
        printf("warp_idx is %d, lane_predicate is %d\n", warp_idx, lane_predicate);
    }
    ...
    // Allocate fragments
    Tensor tCrA = thr_mma.make_fragment_A(tCsA(_,_,_,_0{}));                         // (MMA,MMA_M,MMA_K,PIPE)
    // Allocate descriptors
    Tensor tCrB = thr_mma.make_fragment_B(tCsB);                         // (MMA,MMA_N,MMA_K,PIPE)

    using SmemCopyAtomA = Copy_Atom<SM75_U32x4_LDSM_N, TA>;
    ...
    //Tensor tSsA = smem_thr_copy_Q.partition_S(cute::as_position_independent_swizzle_tensor(sA));
    ...
    CUTE_STATIC_ASSERT_V(size<1>(tCsA_copy_view) == size<1>(tCrA_copy_view));
    CUTE_STATIC_ASSERT_V(size<1>(tCrA) == size<1>(tCrC));
    CUTE_STATIC_ASSERT_V(size<1>(tCsB) == size<2>(tCrC));
    CUTE_STATIC_ASSERT_V(size<2>(tCsA) == size<2>(tCsB));
    CUTE_STATIC_ASSERT_V(size<3>(tCsA) == size<3>(tCsB));
    CUTE_STATIC_ASSERT_V(size<2>(sA) == size<2>(sB));
    ...
    if (thread0()) {
        ...
        print("tCsA_copy_view: "); print(tCsA_copy_view); print("\n");
    }
    ...
    while (k_tile_count > -K_PIPE_MAX) {
        ...
        copy(smem_tiled_copy_A, tCsA_copy_view(_, _, _0{}, read_pipe), tCrA_copy_view(_, _, _0{}));

        for (int ki = 0; ki < size<2>(tCrA); ki++) {
            if  (ki < size<2>(tCrA) - 1) {
                copy(smem_tiled_copy_A, tCsA_copy_view(_, _, ki + 1, read_pipe), tCrA_copy_view(_, _, ki + 1));
            }
            warpgroup_arrive();
            cute::gemm(mma, tCrA(_, _, ki), tCrB(_, _, ki, read_pipe), tCrC);
                    ...
                    at::Tensor& o)
{
    ...
    TiledMMA tiled_mma = make_tiled_mma(SM90_64x192x16_F32F16F16_RS<GMMA::Major::K,GMMA::Major::K>{},
        AtomLayoutMNK{});
